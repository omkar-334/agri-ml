...
class CustomDatasetForFFVIT(Dataset):
    def __init__(self, csv_plant, csv_disease, transforms):
        ...
        for x in self.plant_path_list.index:
            ...
            self.total += len(self.data_plant)
...
for epoch in range(num_epochs):
    ...
    print("Current Learning rate: {}".format(optimizer_classifier.param_groups[0]["lr"]))
    ...
    for batch_idx, (images1, labels_cls, images2, labels_dis) in enumerate(tqdm(train_loader)):
        ...
        for x in range(len(labels_cls)):
            if predictions_cls_2[x] == labels_cls[x] and predictions_dis_2[x] == labels_dis[x]:
                correct_train += 1
    ...
    print(f"Original crop acc: {accuracy_train_cls:.4f}")
    print(f"Original Disease acc: {accuracy_train_dis:.4f}")
    print(f"Synthetic crop acc: {accuracy_train_cls_2:.4f}")
    print(f"Synthetic Disease acc: {accuracy_train_dis_2:.4f}")
    print(f"Combined plant disease acc: {accuracy_train:.4f}")
    print(f"Class base loss: {c}")
    print(f"Disease base loss: {d}")
    print(f"classifier class loss: {cd}")
    print(f"classifier disease loss: {cd2}")
    print(f"Total loss: {c + d + cd + cd2}")
    print(f"backward loss: {tcd}")
    ...
    for images, labels_cls, labels_dis in tqdm(test_loader1):
        ...
        # Post-calculation for plant disease identification prediction
        for x in range(len(labels_cls)):
            if predictions_test_cls_2[x] == labels_cls[x] and predictions_test_dis_2[x] == labels_dis[x]:
    ...
    print(f"Original crop acc: {accuracy_test_cls:.4f}")
    print(f"Original Disease acc: {accuracy_test_dis:.4f}")
    print(f"Synthetic crop acc: {accuracy_test_cls_2:.4f}")
    print(f"Synthetic Disease acc: {accuracy_test_dis_2:.4f}")
    print(f"Combined plant disease acc: {accuracy_test_combine:.4f}")
    print(f"Total testing loss: {tcd}")
    ...
    if (epoch + 1) % 1 == 0:
        ...
        torch.save(
            ...
            os.path.join(Save_model_path, f"{model_name}_{epoch + 1}e_{learning_rate_classifier}L_{accuracy_train:.4f}_{accuracy_test_combine:.4f}_FFViT_model.pth"),
        )

        print("Saving optimizer")
        torch.save(
            optimizer_classifier.state_dict(),
            os.path.join(Save_model_path, f"{model_name}_{epoch + 1}e_{learning_rate_classifier}L_{accuracy_train:.4f}_{accuracy_test_combine:.4f}_FFViT_optimizer.pth"),
        )
