...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0.0, proj_drop=0.0, rpe=True):
        super().__init__()
        self.dim = dim
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = WindowAttention(
            dim, window_size=to_2tuple(self.window_size), num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, rpe=rpe
        )
...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.use_multiscale:
            ...
            for layer in self.layers:  # x=[64,3136,96]
                x = layer(x)  # [64,49,768]
            return [x]

    def forward(self, x):  # [64,3,224,224]
        xs = self.forward_features(x)  # x(list:1)
        x_last = self.norm(xs[-1])  # tensor:[64,49,768]  # [B, L, C]
        # SUP
        pool = self.avgpool(x_last.transpose(1, 2))  # pool:tensor[64,768,1]  # [B, C, 1]
        sup = self.head(torch.flatten(pool, 1))  # tensor[64,1000]
        outs = Munch(sup=sup)
